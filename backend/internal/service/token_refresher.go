...
// GeminiTokenRefresher 处理 Gemini OAuth token刷新
type GeminiTokenRefresher struct {
	geminiOAuthService *GeminiOAuthService
}

// NewGeminiTokenRefresher 创建 Gemini token刷新器
...
func (r *GeminiTokenRefresher) Refresh(ctx context.Context, account *model.Account) (map[string]any, error) {
	...
	// 获取现有的 client_id 和 client_secret
	clientID := account.GetCredential("client_id")
	clientSecret := account.GetCredential("client_secret")

	// 使用服务提供的方法构建新凭证
	newCredentials := r.geminiOAuthService.BuildAccountCredentials(tokenInfo, clientID, clientSecret)
