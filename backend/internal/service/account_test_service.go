...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if account.IsOAuth() {
		...
		// 账号已被探测为不支持 Responses（如 DeepSeek/Kimi 等）时，走 CC 测试路径。
...
// testOpenAIChatCompletionsConnection tests OpenAI-compatible providers that only
// support /v1/chat/completions (not /v1/responses).
func (s *AccountTestService) testOpenAIChatCompletionsConnection(
	c *gin.Context, ctx context.Context, account *Account,
